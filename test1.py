...
'''driver = webdriver.Firefox()
driver.get("https://www.cardekho.com/carmodels/Toyota/Toyota_Innova")'''

driver.find_element_by_css_selector("a[href*='specification']").click()

variants = driver.find_elements_by_xpath("//select[@id='carvarinatid']//option")
for op in range(len(variants)):

    Select(driver.find_elements_by_xpath("//select[@id='carvarinatid']")[0]).select_by_index(op)
    category = {}
